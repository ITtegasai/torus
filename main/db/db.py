...
class Connection:
    ...
    def __init__(self):
        try:
            ...
            #return connection
            conn = self.selectWhereCase("select case when pg_is_in_recovery() then 0 else 1 end as host_status;")
            if conn[0][0] == 0:
                self.connection = psycopg2.connect(
                    user=os.getenv("DB_USER"),
                    password=os.getenv("DB_PASSWORD"),
    ...
    def select(self, table: str, where_key: str = "", where_value: str = "", key: list = "*"):
        try:
            ...
            arr = []
            if where_key != "":
                cursor.execute("SELECT {3} FROM {0} WHERE {1} = '{2}' ORDER BY id ASC;".format(table, where_key, where_value, ", ".join(map(str, key))))
                arr = cursor.fetchall()
            else:
                cursor.execute("SELECT {1} FROM {0} ORDER BY id ASC;".format(table, ", ".join(map(str, key))))
                arr = cursor.fetchall()
            cursor.close()
            cursor = self.connection.cursor()
