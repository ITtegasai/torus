...
import bg1 from "../../assets/images/paper.png";
import bg3 from "../../assets/images/coins.png";
import { WSContext } from "../../context/WebSocketContext";
import { useNavigate } from "react-router-dom";
import { requisitesTorus } from "../../../requisitesTorus";
import dayjs from "dayjs";
import qr from "../../assets/qr/qr_torus.png";
...
export default function Personal() {
  ...
  const { cabinet, user } = useContext(WSContext);
  ...
  const maxStocks = 10;
  const currentStocks = cabinet?.count || 0;
  ...
  const buyPackage = async (summ, count, type, showingSumm) => {
    ...
    try {
      setIsLoading(true);
      const data = await getDocs(token, summ, count, type);
  ...
  const confirmBuy = async () => {
    try {
      ...
      setIsLoading(true);
      const data = await buy(token, packageData.current.summ, packageData.current.count, packageData.current.type);
        ...
        <div className={style.topBlock}>
          <div className={style.dividend}>
            ...
            <div className={style.dividendCount}>
              <p>{cabinet?.dividend || 0} ₽</p>
              <Icon width={24} name="monets" style={{ color: "var(--color-accent)" }} />
            </div>
          </div>
          <div className={style.dividend}>
            <h5 className={style.dividendTitle}>Стоимость СФО* на одну акцию</h5>
            <div className={style.dividendCount}>
              <p>{cabinet?.current_price || 0} ₽</p>
              <Icon width={24} name="monets" style={{ color: "var(--color-accent)" }} />
            </div>
            <p
              style={{
          ...
          <div className={style.stocks}>
            <div className={style.stocksHeader}>
              <div className={style.stocksRemained}>{currentStocks >= 10 ? "Акционер" : "Осталось до акционера"}</div>
            </div>
            <div className={style.stocksBought}>
              Куплено акций <span>{currentStocks}</span>
              {currentStocks < 10 && <span>/{maxStocks}</span>}
            </div>
        ...
        <div className={style.buyStocks}>
          {currentStocks < 10 ? (
            <BuyStocksBlock
              ...
              bgSrc={bg1}
              min={1}
              onClick={(summ, count) => buyPackage(summ, count, 0, summ * cabinet?.current_price)}
            />
          ) : (
            <BuyStocksBlock
              ...
              bgSrc={bg1}
              min={0}
              onClick={(count, summ) => buyPackage(summ, count, 0, count)}
            />
          )}
          {/* <BuyStocksBlock
          ...
          {!cabinet?.fix_timestamp || 0 ? (
            <BuyStocksBlock
              ...
              min={10}
              onClick={(summ, count) => buyPackage(summ, count, 3, summ * cabinet?.current_price)}
            />
          ) : (
            <BuyStocksBlock
              title="Рассрочка"
              smallTitle={`Рассрочка до ${dayjs(cabinet?.fix_timestamp * 1000).format("DD.MM.YYYY")}`}
              label="Сумма"
              bgSrc={bg3}
