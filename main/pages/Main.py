...
class Main:
    def get(self, uid, tx_type = 0, limit = 10, offset = 0, filter = None):
        ...
        if user:
            if user["result"].fix_timestamp2 != None:
                ...
                fix_timestamp = datetime.strptime(user["result"].fix_timestamp2, '%Y-%m-%d').date() + relativedelta(months=1)
                fix_timestamp = calendar.timegm(fix_timestamp.timetuple())
            
        
        current_price = conn.select("packages", "id", "2")
        current_price = current_price["result"]
        current_price = float(current_price.price)
        
            
                
        tr = conn.selectWhereStr(f"SELECT SUM(amount), SUM(amount/price) FROM transactions WHERE user_id = (SELECT id FROM users WHERE uid = '{uid}') and type = 0 and status = 3 and package_id = 2;")
